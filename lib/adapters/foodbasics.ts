...
export async function searchFoodBasics(item: ListItem, postal: string): Promise<StorePrice> {
  ...
  if (!apiKey) {
    ...
    return {
      ...
      url: `https://www.foodbasics.ca/search?search-bar=${encodeURIComponent(item.raw_text)}`,
    };
  }

  try {
    const searchQuery = encodeURIComponent(item.raw_text);
    const url = `https://www.foodbasics.ca/search?search-bar=${searchQuery}`;
    ...
    return {
      ...
      size: size || undefined,
      url: productUrl ? `https://www.foodbasics.ca${productUrl}` : undefined,
    };
  } catch (error) {
    console.error(`Error searching Food Basics for "${item.raw_text}":`, error);
    return {
      price: 0,
      available: false,
      product_name: item.raw_text,
